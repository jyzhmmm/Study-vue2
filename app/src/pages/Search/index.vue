...
export default {
  ...
  methods: {
    ...
    removeCategoryName() {
      // 置空参数，还需向服务器发请求
      // 带给服务器的参数都是可有可无的，如果属性值设置为空的字符串，还是会把字段带给服务器
      // 把字段设置为undefined,当前这个字段就不会带给服务器，优化性能
  ...
  beforeMount() {
    // Object.assign ES6新增语法，合并对象，后面的对象会把前面的对象属性值覆盖
    Object.assign(this.searchParams, this.$route.query, this.$route.params);
  },
  mounted() {
    this.getData();
